namespace Sitecore.Commerce.Sample.Console
{
    ...
    public static class OrdersUX
    {
        ...
        public static void RunScenarios()
        {
            using (new SampleScenarioScope("Orders UX"))
            {
                try
                {
                    ...
                    _orderId = Scenarios.Simple2PhysicalDigitalItems.Run(jeff.Context);
                    ...
                    _orderId = Scenarios.SimplePhysicalDigitalItemsFederatedPayment.Run(bob.Context);
                    _orderId.Should().NotBeNull();
                    HoldOrder(_orderId);
                    ...
                    _orderId = Scenarios.SimplePhysical2Items.Run(jeff.Context);
                    _orderId.Should().NotBeNull();
                    HoldOrder(_orderId);
        ...
        private static void SetFulfillment_Digital()
        {
            using (new SampleMethodScope())
            {
                var orderId = Scenarios.BuyGiftCards.Run(new AnonymousCustomerJeff().Context);
                orderId.Should().NotBeNull();
                HoldOrder(orderId);
        ...
        private static void SetFulfillment_ShipToMe()
        {
            using (new SampleMethodScope())
            {
                var orderId = Scenarios.Buy3Items.Run(new AnonymousCustomerJeff().Context);
                orderId.Should().NotBeNull();
                HoldOrder(orderId);
        ...
        private static void VoidPayment_Federated(string orderId = "")
        {
            using (new SampleMethodScope())
            {
                ...
                var order = Orders.GetOrder(container, orderId);
                var paymentId = order.Components.OfType<Sitecore.Commerce.Plugin.Payments.FederatedPaymentComponent>()
                    .FirstOrDefault()
                    ?.Id;
        ...
        private static void RefundPayment_Federated()
        {
            using (new SampleMethodScope())
            {
                ...
                var orderid = Scenarios.SimplePhysical2Items.Run(new AnonymousCustomerBob().Context);
                var order = Orders.GetOrder(container, orderid);
                var paymentId = order.Components.OfType<Sitecore.Commerce.Plugin.Payments.FederatedPaymentComponent>()
                    .FirstOrDefault()
                    ?.Id;
