namespace Sitecore.Commerce.Sample.Console
{
    ...
    public static class Carts
    {
        ...
        private static readonly Sitecore.Commerce.Engine.Container AwShopsContainer = AwShopperContext.ShopsContainer();
        private static readonly Sitecore.Commerce.Engine.Container HabitatShopsContainer = new AnonymousCustomerSteve().Context.ShopsContainer();
            ...
            using (new SampleMethodScope())
            {
                ...
                result = Proxy.DoCommand(
                        AwShopperContext.ShopsContainer().UpdateListPrices(
                            ...
                            new List<Money> { updatedListPrice }));
                ...
                result = Proxy.DoCommand(
                    AwShopperContext.ShopsContainer().UpdateListPrices(
                        "Adventure Works Catalog|AW475 14|",
                        new List<Money> { originalListPrice }));
                            result.Should().NotBeNull();
                            result.Messages.Any(m => m.Code.Equals("error", StringComparison.OrdinalIgnoreCase)).Should().BeFalse();
            }
        }
